/* Totus Central · RBAC operativo
   Una sola capa semántica para separar supervisión operativa de administración crítica. */
(function(){
  ...
  function asAdminGate(fn,args,ctx){
    const original=window.isAdmin;window.isAdmin=()=>true;
    try{return fn.apply(ctx,args)}finally{window.isAdmin=original}
  }
  function gate(name,allowed){
    const original=window[name];if(typeof original!=='function')return;
    window[name]=function(...args){if(!allowed())return original.apply(this,args);return asAdminGate(original,args,this)};
  }
  function renderGate(name,allowed,{suppress=[]}={}){
    const original=window[name];if(typeof original!=='function')return;
    window[name]=function(...args){if(!allowed())return original.apply(this,args);
      const saved={},admin=window.isAdmin;window.isAdmin=()=>true;
      for(const key of suppress){saved[key]=window[key];window[key]=()=>''}
      try{return original.apply(this,args)}finally{window.isAdmin=admin;for(const key of suppress)window[key]=saved[key]}
    };
  }
  ...
  gate('adminAddAttendance',manager);gate('editAttendance',manager);
  gate('editTaskTime',manager);
  gate('editPurchase',manager);gate('setPurchaseStatus',manager);gate('cancelPurchase',manager);gate('savePurchase',manager);

  /* Encargado ve el equipo de su ámbito pero no controles administrativos sobre personas. */
  ...
  /* Navegación según capacidad, no según un booleano admin/no-admin. */
  window.renderSubnav=function(){
    let tabs=[];
    if(state.root==='tasks'){
      if(systemAdmin())tabs=[['mine','Mi panel'],['overview','Equipo'],['calendar','Calendario'],['tasks','Tareas'],['schedule','Programación'],['stores','Centros y proyectos'],['attendance','Fichajes'],['reports','Informes']];
      else if(manager())tabs=[['mine','Mi panel'],['overview','Equipo'],['calendar','Calendario'],['tasks','Tareas'],['schedule','Programación'],['attendance','Fichajes'],['reports','Informes']];
      else if(supervisor())tabs=[['mine','Mi panel'],['overview','Equipo'],['calendar','Calendario'],['tasks','Tareas'],['schedule','Horarios'],['attendance','Fichajes'],['reports','Informes']];
      else tabs=[['mine','Mi panel'],['calendar','Calendario'],['tasks','Mis tareas'],['schedule','Mi horario'],['attendance','Mis fichajes'],['reports','Mis informes']];
  ...
  if(typeof routedRoot==='function')window.goRoot=function(root,sub=null){
    if(root==='users'&&!systemAdmin()){notifyMsg('Usuarios y permisos es exclusivo de administración.','warn');root='tasks';sub='mine'}
    if(root==='logs'&&sub==='maintenance'&&!systemAdmin()){notifyMsg('Backups y mantenimiento son exclusivos de administración.','warn');sub='general-log'}
    return routedRoot.call(this,root,sub);
  };
  const routedSub=window.goSub;
  if(typeof routedSub==='function')window.goSub=function(sub){
    if(state.root==='tasks'&&sub==='stores'&&!systemAdmin()){notifyMsg('Centros y proyectos se configura desde administración.','warn');sub='mine'}
    if(state.root==='logs'&&sub==='maintenance'&&!systemAdmin()){notifyMsg('Backups y mantenimiento son exclusivos de administración.','warn');sub='general-log'}
    return routedSub.call(this,sub);
  };

  function paintRoleVisibility(){
    const users=document.querySelector('.nav button[data-root="users"]');if(users)users.classList.toggle('hidden',!systemAdmin());
    document.body.dataset.totusRole=currentRole();
    renderSubnav();
  }
