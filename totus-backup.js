...
(function(){
  ...
  async function listBucketPaths(bucket,prefix='',depth=0){
    ...
    while(true){
      ...
      const rows=data||[];
      for(const item of rows){const path=prefix?`${prefix}/${item.name}`:item.name;if(item.id||item.metadata)out.push(path);else out.push(...await listBucketPaths(bucket,path,depth+1))}
  ...
  window.runLoadedBackupRestore=async function(mode){
    ...
    closeModal();
    const ok=await askConfirm({title:exact?'Rollback exacto':'Recuperar backup',message:exact?'Se creará primero una copia completa del estado actual. Después Totus reemplazará datos y archivos por el contenido exacto del backup.':'Se creará primero una copia completa del estado actual y después se fusionará la información del backup sin borrar datos posteriores.',confirmText:exact?'EJECUTAR ROLLBACK':'RESTAURAR',danger:exact,requireText:exact?'ROLLBACK EXACTO':'RESTAURAR'});if(!ok)return;
    ...
    try{
      ...
      if(exact){
        notifyMsg('Sincronizando archivos…');const buckets=Number(b.version)>=2?BACKUP_FILE_BUCKETS:['task-evidence','internal-library'];for(const bucket of buckets)await syncBucketToBackup(bucket,b.files||[]);
      }
      await uploadBackupFiles(b);
      if(exact){
        const emails=(b.data.team_members||[]).map(x=>x.email).filter(Boolean),ids=(b.data.profiles||[]).map(x=>x.id).filter(Boolean);const fin=await sb.rpc('admin_finalize_exact_restore',{p_member_emails:emails,p_profile_ids:ids});if(fin.error)throw new Error(fin.error.message);
      }
