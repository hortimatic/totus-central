/* Totus Central · Backup y rollback V3
   Copia completa de datos, asignaciones de centros/proyectos y archivos.
   Compatible con backups V1/V2 y rollback exacto seguro. */
(function(){
  ...
  const V3_VERSION=3;
  const tablesForVersion=v=>Number(v)>=3?V3_TABLES:LEGACY_TABLES;
  const bucketsForVersion=v=>Number(v)>=2?BACKUP_FILE_BUCKETS:['task-evidence','internal-library'];
  ...
  window.buildPortableBackup=async function(){
    ...
    const data={};for(const table of V3_TABLES)data[table]=await readBackupTable(table);
    const files=await collectBackupFiles(data),rows=backupRowCount({data}),payload={format:'totusbackup',version:V3_VERSION,created_at:new Date().toISOString(),created_by:me().id,scope:'full',manifest:{tables:V3_TABLES.length,rows,files:files.length,buckets:[...new Set(files.map(f=>f.bucket))],features:['workspace_assignments_v3','auth_preflight','exact_rollback'],excludes:['Archivo histórico de backups privados (evita copias recursivas)','Contraseñas de Supabase Auth (no exportables)']},data,files};
    payload.checksum=await sha256Text(JSON.stringify(payload));return payload;
  };

  window.validateBackupPayload=function(obj){
    const version=Number(obj?.version);
    if(!obj||obj.format!=='totusbackup'||![1,2,3].includes(version))throw new Error('Formato o versión no reconocidos');
  ...
  async function uploadBackupFiles(b){
    const allowedBuckets=new Set(bucketsForVersion(b.version));
    for(const f of b.files||[]){if(!allowedBuckets.has(f.bucket)||!f.path)continue;const blob=new Blob([base64ToBytes(f.data)],{type:f.type||'application/octet-stream'}),up=await sb.storage.from(f.bucket).upload(f.path,blob,{upsert:true,contentType:f.type||undefined});if(up.error)throw new Error(`Archivo ${f.bucket}/${f.path}: ${up.error.message}`)}
  }

  async function restoreTables(b){
    for(const table of tablesForVersion(b.version)){const rows=b.data?.[table];if(!Array.isArray(rows))throw new Error(`Bloque ausente: ${table}`);if(!rows.length)continue;const {error}=await sb.rpc('admin_restore_backup_table',{p_table:table,p_rows:rows});if(error)throw new Error(`${table}: ${error.message}`)}
  }

  async function rebuildLegacyAssignmentsIfNeeded(b){
    if(Number(b.version)>=3)return;
    const {error}=await sb.rpc('admin_rebuild_member_locations_legacy');if(error)throw new Error('No se pudieron reconstruir las asignaciones de centros del backup antiguo: '+error.message);
  }
  ...
  async function verifyRestoredBackup(b,exact){
    ...
    const fileIssues=[];
    for(const bucket of bucketsForVersion(b.version)){const expected=new Set((b.files||[]).filter(f=>f.bucket===bucket).map(f=>f.path)),actual=new Set(await listBucketPaths(bucket));if(exact&&(actual.size!==expected.size||[...expected].some(p=>!actual.has(p))))fileIssues.push(`${bucket}: ${actual.size}/${expected.size}`);if(!exact&&[...expected].some(p=>!actual.has(p)))fileIssues.push(`${bucket}: faltan archivos`)}
    if(tableIssues.length||fileIssues.length)throw new Error(`Verificación incompleta. ${[...tableIssues,...fileIssues].slice(0,8).join(' · ')}`);
    return{tables:expectedTables.length,rows:backupRowCount(b),files:backupFileCount(b)};
  }
  ...
  window.runLoadedBackupRestore=async function(mode){
    ...
    closeModal();const ok=await askConfirm({title:exact?'Rollback exacto':'Recuperar backup',message:exact?'Totus creará primero una copia V3 completa del estado actual. Después reemplazará datos y archivos por el backup cargado y verificará el resultado.':'Totus creará primero una copia V3 completa del estado actual y después fusionará el backup sin borrar datos posteriores.',confirmText:exact?'EJECUTAR ROLLBACK':'RESTAURAR',danger:exact,requireText:exact?'ROLLBACK EXACTO':'RESTAURAR'});if(!ok)return;
    ...
    try{
      ...
      notifyMsg('Restaurando datos…');await restoreTables(b);await rebuildLegacyAssignmentsIfNeeded(b);
      if(exact){notifyMsg('Sincronizando archivos…');for(const bucket of bucketsForVersion(b.version))await syncBucketToBackup(bucket,b.files||[])}
      await uploadBackupFiles(b);
      if(exact){const emails=(b.data.team_members||[]).map(x=>x.email).filter(Boolean),ids=(b.data.profiles||[]).map(x=>x.id).filter(Boolean),fin=await sb.rpc('admin_finalize_exact_restore',{p_member_emails:emails,p_profile_ids:ids});if(fin.error)throw new Error(fin.error.message)}
      notifyMsg('Verificando integridad…');const verification=await verifyRestoredBackup(b,exact);await logRestore(mode,b,verification);await reloadTeamData();state.backupLoaded=null;state.backupLoadedName='';state.backupLoadedText='';renderGeneralLog();notifyMsg(exact?'Rollback exacto completado y verificado.':'Recuperación completada y verificada.');
  ...
  window.backupCoverageModal=function(){modal(`<div class="section-head"><div><div class="eyebrow">Backup portable V3</div><h3>Qué queda protegido</h3></div><button class="ghost" onclick="closeModal()">Cerrar</button></div><div class="backup-coverage"><div><b>Equipo y permisos</b><span>Usuarios, perfiles, roles y asignaciones de cada persona a centros/proyectos.</span></div><div><b>Configuración</b><span>Centros/proyectos, reglas, horarios, temporadas, excepciones, festivos, categorías y objetivos.</span></div><div><b>Operativa e histórico</b><span>Tareas, tramos, actividades laborales, fichajes, pausas, compras, vacaciones, consultas y auditorías.</span></div><div><b>Pricing</b><span>Marcas, familias, productos, variantes, proveedores, competencia, consultas e históricos.</span></div><div><b>Archivos reales</b><span>Evidencias de tareas, biblioteca/documentos internos y avatares.</span></div><div><b>Rollback</b><span>Copia previa automática, verificación Auth, compatibilidad V1/V2 y comprobación final de datos/archivos.</span></div></div><div class="note" style="margin-top:9px"><b>Exclusiones intencionadas:</b> el almacén histórico de backups privados no se copia dentro de sí mismo y las contraseñas Auth no son exportables. Todo el estado operativo y la configuración de Totus sí quedan incluidos.</div>`)};
})();
