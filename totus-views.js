...
(function(){
  ...
  function purchaseAlertCard(rows){
    const now=Date.now(),oldPending=rows.filter(x=>x.status==='pending'&&(now-new Date(x.created_at))/86400000>3),missingRef=rows.filter(x=>x.status!=='cancelled'&&(!x.external_reference||x.purchase_kind==='supplier_order'&&!x.source_name)),cancelled=rows.filter(x=>x.status==='cancelled'),corrected=(db.purchaseAudit||[]).filter(a=>a.action==='financial_correction'&&rows.some(p=>p.id===a.purchase_id));
    return card('Alertas de control',`<div class="ref-alert-list"><div><span>Pendientes > 3 días</span><b class="badge ${oldPending.length?'badb':'ok'}">${oldPending.length}</b></div><div><span>Sin referencia/proveedor suficiente</span><b class="badge ${missingRef.length?'warnb':'ok'}">${missingRef.length}</b></div><div><span>Canceladas</span><b class="badge ${cancelled.length?'warnb':'ok'}">${cancelled.length}</b></div><div><span>Correcciones económicas auditadas</span><b class="badge ${corrected.length?'warnb':'ok'}">${corrected.length}</b></div></div>`,{hint:'Solo lo que necesita revisión.',className:'ref-span-2'});
  }
